...
function DisplayTags({ model, setModel, questions, setQuestions, pageTitle, setActiveLink, userInfo, setUserInfo }) {
    // const [questions, setQuestions] = useState([]);
    // const [tags, setTags] = useState([]);
    const tags = model.tags
    // console.log(questions);
    const [sortType, setSortType] = useState("newest");
    const [sortingQ,setSortingQ] = useState(questions)
    ...
    function handleSort(type) {
        ...
        if (type === "newest") {
            ...
            sortedQuestions = [...questions].filter((q) => q.answers.length === 0);
        }
        setSortingQ(sortedQuestions);
    }
                    ...
                    <div className="questions-container">
                        ...
                        <ul className='total-questions-line'>
                            <li className='total-questions'>
                                <p>Total questions: {sortingQ.length}</p>
                            </li>
                        ...
                        <div className="questions-list">
                            {sortingQ && sortingQ.map((q) => (
                                <div key={q._id} >
                                    ...
                                    <div key={q.id} className="question-name" onClick={() => handleSelectedQuestion(q)}>
                                        {q.title}
                                    </div>
                                    ...
                                    <p className="tagsLine">
                                        {q.tags.map((tagId, index) => (
                                            ...
                                            <span key={tagId} className="tagsBg">{tags.map((tag) => (tag._id) === tagId ? tag.name : "")}</span>
                                        ))}
                                    </p>
                                </div>
                            ))}
